...
import { MdAdd, MdSearch, MdVideoLibrary } from 'react-icons/md'
...
export default function VideosPage() {
  ...
  const recentlyViewed = useMemo(
    () => [...mockVideos].sort((a, b) => b.lastOpenedAt - a.lastOpenedAt).slice(0, 6),
    []
  )
  ...
  const filteredVideos = useMemo(() => {
    ...
    return mockVideos.filter((v) => v.name.toLowerCase().includes(q))
  }, [searchQuery])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-800">Videos</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        ...
        <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-thin">
          {recentlyViewed.map((v) => (
            <VideoCard key={v.id} video={v} onClick={setSelectedVideo} compact />
          ))}
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
          <h2 className="text-base font-semibold text-gray-800">All Videos</h2>
          <div className="relative w-full sm:w-64">
            <MdSearch size={18} className="absolute left-3 top-1/2 -translate-y-1/2 text-textMuted" />
            <input
              type="text"
        ...
        {filteredVideos.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mb-4">
              <MdVideoLibrary size={28} className="text-textMuted" />
            </div>
            <p className="text-gray-500 font-medium">No videos found</p>
            <p className="text-sm text-textMuted mt-1">
              {searchQuery ? `No results for "${searchQuery}"` : 'No videos in this folder yet.'}
            </p>
          </div>
        )}
      </div>
